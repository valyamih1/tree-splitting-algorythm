...
def dynamic_tree_splitting(old_N, kmax):
    max_rao = 50
    k_max = kmax - 1
    ...
    for i in range(max_time):
        ...
        for j in range(len(exact_devices)):
            ...
            if counter >= distribution_n_i[i]:
                break
                # if successful_requests[i] < exact_devices[j][0]:
                #     backoff = random.randint(1,4)
                #     distribution_n_i[i+backoff] += 1
                #     exact_devices[j] = (exact_devices[j][0], exact_devices[j][1], 1 + exact_devices[j][2], random.randint(1,4))
                #
                # else:
                #     exact_devices[j] = (exact_devices[j][0], exact_devices[j][1], 0 - exact_devices[j][2])
