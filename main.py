...
def dts_rach_graphs():
    ...
    rates = []
    for i in K:
        th, delay, rate = DTS_RACH.dynamic_tree_splitting(i)
        throughputs.append(th)
        delays.append(delay)
        rates.append(rate)
    plt.figure(1)
    plt.plot(K, throughputs, label='Mean Throughput', marker=".", linewidth=2, markersize=7, linestyle='--')
    plt.xlabel('Number of devices')
    ...
    plt.figure(2)
    plt.plot(K, delays, label='Mean delay', marker=".", linewidth=2, markersize=7, linestyle='--')
    plt.xlabel('Number of devices')
    ...
    plt.figure(3)
    plt.plot(K, rates, label='Success rate', marker=".", linewidth=2, markersize=7, linestyle='--')
    plt.xlabel('Number of devices')
